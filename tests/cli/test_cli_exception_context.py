...
from registers.cli.exceptions import CommandExecutionError, RegistrationError


def test_framework_error_supports_structured_context() -> None:
    err = RegistrationError("cli failure", request_id="abc123", operation="run")
    payload = err.to_dict()

    assert payload["type"] == "RegistrationError"
