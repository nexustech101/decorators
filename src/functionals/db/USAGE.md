# functionals.db Usage

`functionals.db` is a lightweight persistence layer for Pydantic models built on SQLAlchemy.
...
```python
from pydantic import BaseModel
from functionals.db import database_registry, HasMany, BelongsTo, HasManyThrough
...
- logs are emitted under `functionals.db.*` so production logging/observability pipelines can capture failures cleanly
