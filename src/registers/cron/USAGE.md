# `registers.cron` Usage

`registers.cron` is a decorator-driven scheduler and automation module built for:
...
`registers.cron` supports two compatible registration styles:
- module-level convenience (`import registers.cron as cron`)
